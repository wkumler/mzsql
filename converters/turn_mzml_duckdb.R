...
turn_mzml_sqlite <- function(ms_files, outfile, ordered=NULL){
  ...
  file_info <- msdata$metadata[,setdiff(names(msdata$metadata), "config_data"),with = FALSE]
  ...
  if(!is.null(ordered)){
    if(ordered=="mz"){
      msdata$MS1 <- msdata$MS1[order(mz)]
      msdata$MS2 <- msdata$MS2[order(premz)]
    }
    if(ordered=="rt"){
      scan_info <- scan_info[order(rt)]
      msdata$MS1 <- msdata$MS1[order(rt)]
      msdata$MS2 <- msdata$MS2[order(rt)]
    }
  }
  
  conn <- dbConnect(duckdb::duckdb(), outfile)
...
turn_mzml_sqlite(ms_files, "demo_data/msdata.duckdb")
