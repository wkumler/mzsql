...
data.frame(
  ...
  gtsave(filename = "manuscript/figures/fileinfo_table.png", vwidth=1000)
...
data.frame(
  ...
  tab_style(style = cell_text(align = "left"), locations = cells_title()) %>%
  gtsave(filename = "manuscript/figures/scaninfo_table.png", vwidth=1000)
...
data.frame(
  ...
  tab_style(style = cell_text(align = "left"), locations = cells_title()) %>%
  gtsave(filename = "manuscript/figures/ms1_table.png", vwidth=1000)
...
data.frame(
  ...
  tab_style(style = cell_text(align = "left"), locations = cells_title()) %>%
  gtsave(filename = "manuscript/figures/ms2_table.png", vwidth=1000)
```
...
*Supplemental figure XX: (TBD) Additional possible extensions to the database schema for retention time correction, feature detection and correspondence, ion mobility, and imagining.*
...
Timing data was obtained on a Lenovo XX blah XX with [cores] and [memory].
...
```{r singlefile_fig}
all_timings <- read_csv("manuscript/data/singlefile_times.csv") %>% 
  ...
  mutate(query=factor(query, levels=c("ms1_spec", "chrom", "rtrange",
                      ...
                      labels=c("Full scan", "Chromatogram", "RT Range",
                               "MS/MS scan", "Precursor search",
                               "Fragment search")))
all_sizes <- read_csv("manuscript/data/file_sizes.csv") %>%
  mutate(method=factor(method, levels=method_names))
...
ggsave("manuscript/figures/singlefile_fig.png", width = 6.5, height=5)

```

![](figures/singlefile_fig.png)

*Figure 2: Query time for the six data extraction methods and the associated file sizes for all 13 methods explored in this paper. The left six panels show boxplots representing the time required in seconds to extract an full scan spectrum, an ion chromatogram, all data within a retention time range, an MS/MS scan, the fragments of a specified precursor, and all precursors with a specified fragment. The error in the boxplot is composed of timing information for 10 repeated queries, each of a different target scan number, retention time (RT), or m/z. The right panel shows a barplot of the size on disk in megabytes (MB) occupied by each file type.*

```{r singlefile_supp}
fun_levels = c("<span style='color:#a41118'>Including initial load</span>", 
               "<span style='color:#0b505c'>Extraction only</span>")
init_outside_gp <- read_csv("manuscript/data/singlefile_supp.csv") %>%
...
pyopenms_precomp_gp <- read_csv("manuscript/data/pyopenms_precomp.csv") %>%
...
ggsave("manuscript/figures/supp_preload_fig.png", width = 6.5, height=5)
```
...
```{r}
multichrom_times <- read_csv("manuscript/data/multichrom_times.csv")
...
ggsave("manuscript/figures/multichrom_fig.png", width = 6.5, height=5)
```
...
multichrom_times %>%
  ...
  theme_bw() +
  theme(legend.position = "none")
ggsave("manuscript/figures/supp_multichrom_facets.png", width = 6.5, height=5)
```
...
read_csv("manuscript/data/dbloopjoinuni_times.csv") %>%
  ...
  theme(legend.position = "inside", legend.justification.inside = c(0.5, 0.95),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")
ggsave("manuscript/figures/supp_dbloopjoinuni.png", width = 6.5, height=5)
```
...
read_csv("manuscript/data/multifile_timings.csv") %>%
...
ggsave("manuscript/figures/multifile_fig.png", width = 6.5, height=3.5)
```
