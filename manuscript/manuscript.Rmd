---
title: "Storing mass-spectrometry data in simple databases enables flexible and intuitive exploration without time or space penalties"
output: 
  word_document:
...
Mass spectrometry (MS) generates large datasets that are stored in increasingly optimized and complex file types, demanding technical expertise to extract information rapidly and easily. We wondered whether a simple structured query language (SQL) database could hold raw MS data and allow for easily readable queries without incurring major penalties in read time or disk space relative to other popular MS formats. Here, we describe a basic MS schema with intuitive database tables and fields that can outperform other formats for exploratory and interactive analysis according to six data subsets commonly extracted: single scans (both MS^1^ and MS^2^), ion chromatograms, retention time ranges, and fragmentation searches (both precursor and fragment search). Additionally, we compare SQLite, DuckDB, and Parquet implementations and find that they can perform these tasks in under a second even when the files occupy over a gigabyte of data on disk. We believe that this tidy data schema expands nicely to most forms of MS data and offers a way to transparently query datasets while preserving computational performance.

## Key words

Mass spectrometry, data storage, SQL, benchmarking, liquid chromatography, human-centered design, exploratory data analysis


# Introduction

The past fifteen years have seen a proliferation of mass spectrometry (MS) data storage format proposals. While the mzML file type has become the standard open-source format [@Martens2011], its text-based XML structure and emphasis on metadata preservation mean that access times are relatively slow and file sizes are substantially larger than the vendor formats. As a result, various alternatives have been introduced that optimize for read time, disk space savings, or both.

These include direct improvements to the mzML format with indexing [@Rost2015] and better internal encoding of the data [@Bhamber2021], HDF5-based alternatives [@Bhamber2021; @Wilhelm2012; @Bilbao2023; @Tully2020; @Askenazi2017], relational databases [@Shah2010; @Bouyssie2015; @Handy2017; @Yang2022; @Beagley2009], or fully custom alternatives [@Rompp2011; @Lu2022]. Fundamentally, these alternatives exchange ease of use for access speed and/or size on disk with clever compression algorithms and modern data structures that move away from the human-readable format of the mzML. These optimized formats often present a steeper learning curve, as their documentation and cross-language examples remain limited. This steep learning curve, coupled with a lack of support in conversion tools such as Proteowizard’s `msconvert` [@Chambers2012], has slowed widespread adoption of these new formats despite their clear computational advantages. These formats can also be fragile without community support, as maintenance often depends on the original developers (as is the case with YAFMS, Shaduf, and mz5, all of whom have links in their papers that currently redirect to missing webpages). However, we did not discover a method of storing MS data that emphasized simplicity and thereby ease of use.

We wondered whether a rudimentary relational database system would be an effective way to quickly and easily query MS data for exploratory work while reducing the complexity required for such queries. Relational databases are not new for MS workflows (see references above) and compete predominantly with HDF5-based methods. Both of these systems are widely used for big data and can be applied to MS data in a variety of ways, leading to the proliferation of implementations we see today. Both backends provide excellent universality, larger-than-memory support, and rapid access to data, but HDF5-based systems excel at self-description and hierarchical structures [@Askenazi2017] while the relational database model is optimized for multi-table queries using a consistent syntax [@Codd1970]. Relational databases are increasingly seen in MS workflows for both raw and processed data, with SQLite backends now supported in the popular peakpicking software xcms [@Smith2006] via the Spectra package [@Rainer2022] (though in-memory and HDF5 options are also supported) and on MetabolomicsWorkbench [@Sud2016] while the development of MassQL [@Jarmusch2022] demonstrates the increasing comfort that MS analysts have with the adoption of structured query language (SQL).

Relational databases also have several distinct advantages over hierarchical or text-based systems, particularly in performing searches for subsets of data via indices. Importantly, this indexing differs from the byte-offset indexes that already exist in the indexed mzML and HDF5 formats because the search for a particular subset cannot be done efficiently with a byte-offset index when the *m/z* data is encoded (whether by numpress, zlib, or just base64), though access to a particular scan known in advance can be incredibly rapid. Additionally, data from multiple samples can be stored together in a single database table to permit queries of all dataset samples to be performed without looping through each file in turn. This differs from existing formats like mzDB, MzTree, and mzMD and thereby avoids the associated computational overhead and query complexity.

SQL databases also allow mass spectrometrists to access the extensive documentation, continual improvements, and long-term stability produced by the industries who specialize in these. While HDF5 is a common scientific data format, databases are constantly under development by industry titans deeply invested in their maintenance and optimization. Online analytical processing (OLAP) methods are particularly well suited for MS data given their optimization for read speed under the assumption of infrequent transactions, making modern systems such as DuckDB [@Raasveldt2019] or Apache's Parquet formats highly appealing while preserving the familiar file-based serverless approach. These formats are also increasingly of interest to ongoing initiatives such as arcMS [@LeRoux2024] and mzPeak [@VanDenBossche2025].
...
E:
cd mzsql\MTBLS10066
# Manually copy over files via FileZilla or other FTP client
ftp://ftp.ebi.ac.uk/pub/databases/metabolights/studies/public/MTBLS10066
# Pull over all raw files in FILES/RAW_FILES/POS (all 225 of them)


mkdir 20220923_LEAP-POS_QC04
cd 20220923_LEAP-POS_QC04
...
# Repeat for
20220929_LEAP-POS_BL14B
20220923_LEAP-POS_S036
20230303_LEAP02_B2_POS_S054
20221001_LEAP-POS_QC20

```

```{r}
cwd <- getwd()
setwd("E:/mzsql/MTBLS10066")
for(file_i in c("20220923_LEAP-POS_QC04",
                "20220929_LEAP-POS_BL14B", "20220923_LEAP-POS_S036",
                "20230303_LEAP02_B2_POS_S054", "20221001_LEAP-POS_QC20")){
  system(paste("mkdir", file_i))
  system(paste0("cp ", file_i, ".raw ", file_i))
  setwd(file_i)
  system(paste0("msconvert ", file_i, '.raw --filter "peakPicking true 1-"'))
  system(paste0("msconvert --mzMLb ", file_i, '.raw --filter "peakPicking true 1-"'))
  system(paste0("msconvert --mz5 ", file_i, '.raw --filter "peakPicking true 1-"'))
  system(paste0('"C:\\Program Files\\ProteoWizard\\raw2mzDB_0.9.10_build20170802\\raw2mzDB.exe" -i ', file_i, '.raw  -c 1-2 --safeMode'))
  system(paste0('mza -file ', file_i, '.raw'))
  setwd("..")
}
setwd(cwd)
```
...
We randomly sampled five files out of the full dataset for comparison across metrics and file formats (20220923_LEAP-POS_QC04, 20220923_LEAP-POS_S036, 20220929_LEAP-POS_BL14B, 20221001_LEAP-POS_QC20, and 20230303_LEAP02_B2_POS_S054). We sampled 10 random scan numbers using Python's `random.randint` function with a seed of 123 and pulled out the largest ions for chromatogram extraction using a 10 ppm mass range window. Retention time ranges were the highest-intensity retention time of each ion chromatogram plus or minus one minute. Similarly, the largest fragments by intensity were used for the MS/MS metrics with a 10 ppm mass range exclusion window.

Timing was performed via Python's `timeit` library and the `timeit.repeat` function, with the various file formats as the innermost loop to ensure bias over time was distributed equally among function calls. File sizes were estimated using Python's `os` library with `os.path.getsize`. Timing data was primarily obtained on an Intel Xeon CPU with two X5650 (\@2.67 GHz) processors and 24 total cores running Windows 10 Pro (64-bit version). 96 gigabytes (GB) of RAM (DDR3 \@1333 MHz) were available and a solid-state drive was used for disk storage.

We did not exhaustively monitor memory usage, though our heuristic exploration of the timing scripts did not ever indicate that memory was a constraint. To confirm this, we reran the analyses on a newer laptop computer (Intel Core i7-8665U vPro 8^th^ Gen \@1.90GHz with 8 total cores running Windows 11 Home, LPDDR3 \@2133Mhz and solid-state storage).

# Results

We settled on a large dataset of gut microbiota LC-MS files published in Portlock et al. (2025) @Portlock2025 and available on Metabolights under accession number MTBLS10066. These lipid plasma samples were run on a Thermo Fisher Q-Exactive mass spectrometer with heated electrospray ionization source using a ternary solvent gradient between water, methanol, and IPA on a Phenomenex Kinetex F5 column. A variable mass range was used with an Orbitrap resolution of 17,500, targeting the 200-1,000 *m/z* range for the first 8 minutes and then 500-1,250 *m/z* for the remainder of the run. Four precursors were selected for DDA each cycle with an isolation window of 2 Da. Additional details are available in the supplementary information of Portlock et al. (2025) @Portlock2025.

## All existing MS data formats demand a high level of domain knowledge

We were able to obtain or write parsers for seven different existing mass spectrometry (MS) data formats: mzML, mzMLb, mz5, mzDB, MZA, MzTree, and mzMD. Multiple Python packages exist for the mzML data format so we used each of the three dominant packages (pyteomics, pyOpenMS, and pymzml) and compared their timing results as well. We were not able to do the same for the YAFMS and Shaduf file types because these formats appear to be deprecated, with broken or missing repository links, the toffee file type due to its application solely to time-of-flight (TOF) data-independent acquisition (DIA) data, the Aird file type due to its current deprecation in Python and C#, and the UIMF format due to a complete lack of interface documentation.
...
all_timings <- read_csv("data/multifile_times.csv") %>% 
  ...
  mutate(query=factor(query, levels=c("ms1_spec", "chrom", "rtrange",
                      ...
                      labels=c("A. Full scan", "B. Chromatogram", "C. RT Range",
                               ...
                               "F. Fragment search")))
all_sizes <- read_csv("data/multifile_sizes.csv") %>%
  mutate(method=str_replace(method, "pyopenms_2DPeak", "2DPeak")) %>%
  mutate(method=factor(method, levels=method_names))
...
size_barplot <- all_sizes %>%
  ...
  geom_boxplot(aes(x=method, y=file_size/1e6, fill=method), color="black") +
...
singlefile_fig <- time_boxplot + size_barplot + plot_layout(widths = c(3.5, 1))

ggsave(singlefile_fig, file="figures/singlefile_fig.png", width = 6.5, height=5)
ggsave(singlefile_fig, file="figures/singlefile_fig.pdf", width = 6.5, height=5)
```

![](figures/singlefile_fig.png)

*Figure 2: Query time for the six data extraction methods and the associated file sizes for all 13 methods explored in this paper. The left six panels show boxplots representing the time required in seconds to extract a full scan spectrum (A), an ion chromatogram (B), all data within a retention time range (C), an MS/MS scan (D), the fragments of a specified precursor (E), and all precursors with a specified fragment (F). The error in the boxplot is composed of timing information for 10 repeated queries for each of 5 different randomly chosen files, each of a different target scan number, retention time (RT), or m/z. The right panel (G) shows a boxplot of the size on disk in megabytes (MB) occupied by each file type.*
...
These comparisons are not entirely equivalent, as the files do not all store the same information. For example, MzTree and mzMD appear to omit the MS/MS data present in the sample DDA file, a potentially significant source of size reduction. While the exact impact is difficult to quantify, an `msconvert` extraction of the same file containing only MS^1^ scans was 58 MB, representing a 23% reduction in size. The SQLite, DuckDB, and Parquet formats similarly exclude much of the scan and file metadata present in other formats, though the precise fraction of disk space occupied by such metadata is difficult to determine and depends on how “metadata” is defined.

### Memory use and hardware support

We then reran the same tests for a single file on a laptop with 16 gigabytes of memory and more modern CPU (Intel Core i7) and software (Windows 11 Home) to determine whether our specific setup was a factor in the benchmarks we obtained. Laptop timings were consistently about twice as fast as those obtained on the desktop (Supplemental Figure 3), indicating that memory availability was not a major factor and that more modern hardware may be more important in determining query times. We were also able to use this as an opportunity to test the robustness of our installation and found that we could not easily reproduce the exact environment used on the desktop due to incompatibility between a more modern version of HDF5 and the version of h5py used to interface with the mz5 files. However, installation of pyopenms was much smoother on more modern hardware with AVX support and we were able to use modern versions of numpy for the laptop rerun, though breaking changes in the arguments required by the 2DPeak method required rewriting some code and we were still required to step back to Python 3.13 to install via pip because the wheels for 3.14 were not yet built. All other installation processes were functionally identical between the two machines.

```{r laptop_v_desktop_timings, eval=FALSE}
...
bind_rows(ILC_timings, laptop_timings) %>% 
  mutate(machine=c("desktop", "laptop")[as.numeric(factor(machine))]) %>%
  pivot_wider(names_from = "machine", values_from="time") %>%
  # mutate(ratio=desktop/laptop) %>% summarise(ratio=mean(ratio), .by=c(query, method)) %>% 
  # arrange(method) %>% print(n=Inf)
...
ggsave("figures/multichrom_fig.pdf", width = 6.5, height=5)
...
read_csv("data/multifile_timings.csv") %>%
  ...
  labs(x="Database size", y="Time required to extract\na random chromatogram", color=NULL, fill=NULL, shape=NULL) +
  theme_bw() +
...
ggsave("figures/multifile_fig.pdf", width = 6.5, height=3.5)
...
There are many use cases in mass spectrometry (MS) that require a complex integration between file metadata and raw *m/z* and intensity pairs. In these circumstances, existing MS data formats and workflows provide good reproducibility and robust analysis potential. However, a significant number of workflows are more exploratory in nature and would benefit from a simple structure that emphasizes rapid and intuitive data retrieval. Indeed, this manuscript was inspired by our positive personal experience with in-memory tables and their ability to rapidly and flexibly explore the data space. Here, we tested whether tidy databases with two to four tables with just a few columns suit this use case and avoid the major memory constraints of Kumler and Ingalls (2022) [@Kumler2022] without incurring significant time or space penalties and find that they do in fact excel in many ways.

Our experience with existing MS formats revealed that documentation is often sparse and data structures are complex, which can limit the flexibility of use outside the original developer’s workflow. A particular pain point is the way in which MS data is stored because current methods must make trade-offs between simplicity, size, and speed. When we explored the wide range of MS file readers, we found that each method presented challenges that may limit adoption. Alternatives to mzML may appear to offer easy opportunities for improvement given its scan-oriented, XML-based structure that sacrifices speed and size in favor of clarity, but no alternative has yet reached a large audience of active users. Fifteen years of active competition continue to favor the highly explicit format, possibly because users are cautious about adopting complex alternatives without assurances of long-term support.
...
However, scan-based methods were less efficient for chromatogram extraction because these axes are inherently orthogonal to each other. mzDB was competitive with the specifically optimized MzTree and mzMD formats, illustrating its success as an axis-agnostic structure. We were especially impressed with the 2DPeak method in pyopenms when extracting multiple chromatograms, as it had essentially a single setup time cost after which any number of chromatograms could be extracted for free and can therefore be highly recommended for visualization applications. Chromatograms (and retention time range queries) are of course only relevant for chromatography-based workflows but this type of analysis has become increasingly popular, making ion extraction increasingly important [@Ting2015]. MS/MS data support remains limited across the existing formats we tested despite the growing availability of fragmentation data, though our use-case is oriented more towards exploration and discovery instead of comprehensive analysis.

Finally, a complexity penalty must be noted for formats and packages requiring complex installation procedures. Pyopenms appears to be the worst offender here, bindings to C++ libraries requiring us to step back to old Python and numpy versions to successfully access the data on our desktop setup and required direct input from the maintainers, though they were very responsive via Github. MzTree and mzMD had minimal documentation on the installation process consisting of a single README file and have not been updated in years. Similarly, mzDB files were difficult to parse due to its opaque SQLite schema and use of the BLOB encoding type that again lacked examples beyond Java and had to be deduced iteratively. Of course, these methods eventually provided enough information that they could be parsed unlike Aird, toffee, and UIMF.
...
Funding for this project was provided by the Simons Foundation (Grant Number 385428 to A.E.I.).
...
Figure S5: Scatterplot showing time required to extract chromatograms from SQLite, DuckDB, and Parquet databases using different methods denoted by color. Ten replicates are shown for each data point.

# TOC figure

```{r TOC, eval=FALSE}
library(mzml2db)
# mzml2db("../demo_data/180205_Poo_TruePoo_Full1.mzML", db_engine = duckdb::duckdb(), db_name = "../demo_data/180205_Poo_TruePoo_Full1.duckdb")

library(DBI)
conn <- dbConnect(duckdb::duckdb(), "../demo_data/180205_Poo_TruePoo_Full1.duckdb")

chrom_fig <- dbGetQuery(conn, "SELECT * FROM MS1 WHERE mz BETWEEN 118.086 AND 118.087") %>%
  mutate(int=int/1e8) %>%
  RaMS::qplotMS1data() +
  theme_bw() +
  labs(y=NULL, x=NULL) +
  theme(text=element_text(size=8), axis.ticks.length = unit(0.01, "in"))
ggsave(plot = chrom_fig, "figures/TOC_chrom.png", width = 1.5, height=0.85)

spec_fig <- dbGetQuery(conn, "SELECT * FROM MS1 WHERE scan_idx = 1000") %>%
  filter(mz<300) %>%
  mutate(int=int/1e6) %>%
  ...
  labs(y=NULL, x=NULL) +
  theme(text=element_text(size=8),  axis.ticks.length = unit(0.01, "in"))
ggsave(plot = spec_fig, "figures/TOC_spec.png", width = 1.5, height=0.85)

rt_fig <- dbGetQuery(conn, "SELECT * FROM MS1 WHERE rt BETWEEN 7 AND 8") %>%
  filter(mz<119.5) %>%
  filter(mz>115) %>%
  arrange(int) %>%
  ggplot(aes(x=rt, y=mz, color=log10(int))) +
  ...
  labs(y=NULL, x=NULL)
ggsave(plot = rt_fig, "figures/TOC_rtrange.png", width = 1.8, height=1)
