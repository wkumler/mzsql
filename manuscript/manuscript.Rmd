...
Details about the size, number of scans (MS1 and MS2), RT range, etc. for the files
...
mzDB and MZA both had reasonably good documentation, both formats notably providing self-contained extensions to `msconvert` for ease of conversion. However, both converters provide limited coverage, with mzDB notably missing support for Waters and Agilent .d files while MZA currently lacks support for AB Sciex .wiff and Bruker .baf files and is only available via binary executable (.exe), restricting its use to Windows platforms. Additionally, both parsers appear to be unable to separate scans from a polarity-switching experiment or support any of the other filters available natively in `msconvert`, as additional arguments passed to the executable throw errors instead of being passed along to the algorithm.
...
[TODO: note the dependencies required by each of the mzML parsers (and mzapy/mz5?)]
...
time_boxplot <- all_timings %>%
  ggplot() +
  geom_boxplot(aes(x=method, y=time)) +
  facet_wrap(~query) +
  scale_y_log10() +
...
size_barplot <- all_sizes %>%
  ...
  geom_col(aes(x=method, y=file_size/1e6), color="black") +
...
## Timings for a dataset with multiple files 

The single-file, single-metric case discussed above and shown in Figure XX is largely a worst-case scenario for many MS data systems that have a slow initial setup step that makes downstream analysis faster. To compare these systems more fairly to our database schema, we also tested timings across multiple files and queries of more than a single metric.
