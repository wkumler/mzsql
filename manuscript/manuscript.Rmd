...
msconvert 20220921_LEAP-POS_BL01.raw --filter "peakPicking true 1-"
msconvert 20220921_LEAP-POS_BL01.raw --mzMLb --filter "peakPicking true 1-"
msconvert 20220921_LEAP-POS_BL01.raw --mz5 --filter "peakPicking true 1-"
raw2mzDB -i 20220921_LEAP-POS_BL01.raw  -c 1-2 --safeMode
mza -file 20220921_LEAP-POS_BL01.raw
```
...
We settled on a large dataset of gut microbiota LCMS files published in @Portlock2025 and available on Metabolights under accession number MTBLS10066.
...
multifile_data <- read_csv("data/multifile_times.csv") %>%
  mutate(type=factor(type, levels=c("multifile", "singlefile"),
                     labels=c("Single database", "One database per MS file"))) %>%
  mutate(method=factor(method, levels=c("sqlite", "duckdb", "parquet"),
                       labels=c("SQLite", "DuckDB", "Parquet"))) %>%
  mutate(n_file_fact=factor(n_files, levels=c(1, 3, 10, 30, 100), 
                        labels=c("One file", "3 files", "10 files", 
                                "30 files", "100 files")))

multifile_data %>%
  select(method, n_files, time, type) %>%
  mutate(n_files=log10(n_files), time=log10(time)) %>%
  nest(data=c(n_files, time)) %>%
  mutate(lmfit=map(data, ~broom::tidy(lm(.x$time~.x$n_files, offset = .x$n_files)))) %>%
  unnest(lmfit) %>%
  filter(term!="(Intercept)")

xdb_gp <- multifile_data %>%
  ggplot(aes(x=n_files, y=time, color=method, fill=method)) +
  geom_abline(slope = 1, intercept = -5:3) +
  geom_smooth(method="lm", formula=y~x, position=position_dodge(width=0.2), fill="grey50") +
  geom_point(position=position_dodge(width=0.2), color="black", pch=21) +
  facet_wrap(~type, nrow=1) +
  scale_y_log10() +
  scale_x_log10() +
  # scale_color_manual(breaks = method_names, values=method_colors, aesthetics = c("color", "fill")) +
  scale_color_viridis_d(aesthetics = c("color", "fill"), begin = 0.2, end = 0.8) +
  labs(x="Number of files", y="Time (seconds)", color=NULL, fill=NULL) +
  theme_bw() +
  theme(legend.position="inside", legend.justification.inside = c(0.05, 0.95),
        legend.background = element_rect(color="grey80"))

xmeth_gp <- multifile_data %>%
  ggplot() +
  geom_boxplot(aes(x=n_file_fact, y=time, color=type)) +
  facet_wrap(~method, nrow=1) +
  scale_y_log10() +
  scale_color_viridis_d(option = "A", begin = 0.4, end = 0.8) +
  theme_bw() +
  labs(x="Number of files", y="Time (seconds)", color="Database type") +
  theme(legend.position="top")

idx_gp + (xdb_gp/xmeth_gp + plot_layout(heights = c(2,1)))
ggsave("figures/multifile_fig.png", width = 10.5, height=7)
```
