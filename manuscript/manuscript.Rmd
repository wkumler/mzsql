...
^1^ University of Washington, School of Oceanography [Address]
^2^ University of Washington, [Mechanical Engineering]
...
ggsave(singlefile_fig, "figures/singlefile_fig.png", width = 6.5, height=5)
```
...
We then reran the same tests for a single file on a laptop with 16 gigabytes of memory and more modern CPU (Intel Core i7) and software (Windows 11 Home) to determine whether our specific setup was a factor in the benchmarks we obtained. Laptop timings were consistently about twice as fast as those obtained on the desktop (Supplemental Figure 3), indicating that memory availability was not a major factor and that more modern hardware may be more important in determining query times. We were also able to use this as an opportunity to test the robustness of our installation and found that we could not easily reproduce the exact environment used on the desktop due to incompatibility between a more modern version of HDF5 and the version of h5py used to interface with the mz5 files. However, installation of pyopenms was much smoother on more modern hardware with AVX support and we were able to use modern versions of both Python and numpy for the laptop rerun, though breaking changes in the arguments required by the 2DPeak method required rewriting some code. All other installation processes were functionally identical between the two machines.
...
*Supplemental figure 1: Additional possible extensions to the database schema for retention time correction, feature detection and correspondence, ion mobility, and imaging.*
...
```{r singlefile_laptop_ILC_comparison}
ILC_timings <- read_csv("data/singlefile_times.csv") %>% 
  # print(n=Inf)
  mutate(method=str_replace(method, "pyopenms_2DPeak", "2DPeak")) %>%
  mutate(method=factor(method, levels=method_names)) %>%
...
laptop_desktop_comp_gp <- bind_rows(ILC_timings, laptop_timings) %>%
  ggplot() +
  geom_boxplot(aes(x=method, y=time, color=method)) +
...
read_csv("data/dbloopjoinuni_times.csv") %>%
  ...
  theme(legend.position = "inside", legend.justification.inside = c(0.5, 0.95),
        ...
        plot.title.position = "plot", 
        plot.caption.position =  "plot")
ggsave("figures/supp_5_dbloopjoinuni.png", width = 6.5, height=3)
```

![](figures/supp_5_dbloopjoinuni.png)

*Supplemental figure 5: Scatter showing time required to extract chromatograms from SQLite, DuckDB, and Parquet databases using different methods denoted by color. Ten replicates are shown for each data point.*
