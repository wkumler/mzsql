...
random.seed(123)
basename="C:/Users/Will/Documents/mzsql_files/20220923_LEAP-POS_QC04/20220923_LEAP-POS_QC04"
print(basename)
...
init_df.to_csv('data/singlefile_times_laptop.csv', index=False)
...
for target_spec in rand_MS1_scans:
    ...
    for name, suffix, filetype in [function_list[i] for i in [0, 1, 3, 4,5,6,7,10,11,12]]:
        ...
        time_df.to_csv('data/singlefile_times_laptop.csv', mode='a', header=False, index=False)
...
pd.DataFrame(file_sizes.items(), columns=["method", "file_size"]).to_csv("data/file_sizes_laptop.csv", index=False)
