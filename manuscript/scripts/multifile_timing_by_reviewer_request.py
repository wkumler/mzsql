...
init_df = pd.DataFrame(columns=["query", "target", "method","time", "filename"])
init_df.to_csv('data/multifile_times.csv', index=False)

init_sizes = pd.DataFrame(columns=["method", "file_size", "filename"])
...
for basename_index, basename in enumerate(basenames):
    ...
    final_MS1_scan_num = conn.execute("SELECT MAX(id) FROM MS1").fetchone()[0]
    rand_MS1_scans = random.sample(range(1, final_MS1_scan_num), 10)
    final_MS2_scan_num = conn.execute("SELECT MAX(id) FROM MS2").fetchone()[0]
    rand_MS2_scans = random.sample(range(1, final_MS2_scan_num), 10)
    ...
    for target_spec in rand_MS1_scans:
        ...
        for name, suffix, filetype in [function_list[i] for i in [0, 1, 3, 4,5,6,7,10,11,12]]:
            ...
            time_df = pd.DataFrame({"query":["ms1_spec"], "target":[target_spec], "method":[name], "time":time_taken, "filename":[basename]})
            time_df.to_csv('data/multifile_times.csv', mode='a', header=False, index=False)
    ...
    for target_mz in top_masses:
        ...
        for name, suffix, filetype in function_list:
            ...
            time_df = pd.DataFrame({"query":["chrom"], "target":[target_mz], "method":[name], "time":time_taken, "filename":[basename]})
            time_df.to_csv('data/multifile_times.csv', mode='a', header=False, index=False)
    ...
    for target_rt in top_rts:
        ...
        for name, suffix, filetype in function_list:
            ...
            time_df = pd.DataFrame({"query":["rtrange"], "target":[target_rt], "method":[name], "time":time_taken, "filename":[basename]})
            time_df.to_csv('data/multifile_times.csv', mode='a', header=False, index=False)
    ...
    for target_spec in rand_MS2_scans:
        ...
        for name, suffix, filetype in [function_list[i] for i in [0, 1, 3, 4, 5, 6, 7, 10, 11, 12]]:
            time_taken = time_spec(basename, suffix, filetype, target_spec, verbose=True)
            time_df = pd.DataFrame({"query":["ms2_spec"], "target":[target_spec], "method":[name], "time":time_taken, "filename":[basename]})
            time_df.to_csv('data/multifile_times.csv', mode='a', header=False, index=False)
    ...
    for target_pre in top_precursors:
        ...
        for name, suffix, filetype in [function_list[i] for i in [0, 1, 3, 4, 5, 6, 10, 11, 12]]:
            ...
            time_df = pd.DataFrame({"query":["premz"], "target":[target_pre], "method":[name], "time":time_taken, "filename":[basename]})
            time_df.to_csv('data/multifile_times.csv', mode='a', header=False, index=False)
    ...
    for target_frag in top_fragments:
        ...
        for name, suffix, filetype in [function_list[i] for i in [0, 1, 3, 4, 5, 6, 10, 11, 12]]:
            ...
            time_df = pd.DataFrame({"query":["fragmz"], "target":[target_frag], "method":[name], "time":time_taken, "filename":[basename]})
            time_df.to_csv('data/multifile_times.csv', mode='a', header=False, index=False)
    ...
    sizedata = pd.DataFrame(file_sizes.items(), columns=["method", "file_size"])
